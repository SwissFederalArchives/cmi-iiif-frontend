...
export default function Metadata(props: IProps) {
    ...
    if (currentManifest.metadata.length > 0) {
        ...
        for (const metadataItem of currentManifest.metadata) {
            const label = DOMPurify.sanitize(getLocalized(metadataItem.label), sanitizeRulesSet);
            let value = DOMPurify.sanitize(getLocalized(metadataItem.value), sanitizeRulesSet);
    ...
    if (props.showLicense !== false && currentManifest.license) {
        metadataView.push(<div key="termsOfUsage">
            ...
            <div className="aiiif-value"><a href={DOMPurify.sanitize(currentManifest.license, sanitizeRulesSet)}>{currentManifest.license}</a></div>
        </div>);
    }
    ...
    if (currentManifest.homepage) {
        if (currentManifest.homepage.id) {
            const value = DOMPurify.sanitize(currentManifest.homepage.id, sanitizeRulesSet);
            if (UrlValidation.isURL(value)) {
                metadataView.push(
                  <a key={"homepage"} href={value} className="aiiif-download" target="_blank"
                     rel="noopener noreferrer">{DOMPurify.sanitize(getLocalized(currentManifest.homepage.label), sanitizeRulesSet)}
                  </a>
                );
            }
        }
